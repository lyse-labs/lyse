...
- **Batch promotion: 5 deterministic rules into the trusted v1 score (28 → 33 stable).** Five already-shipped `experimental` rules had no recall-suite generators, so they were never measured and stayed reported-only. Added their synthetic generators and measured them — all cleared **both** gates (recall LB **0.901**, precision LB **0.904**, 2026-06-17 run) and are promoted: `a11y/prefers-reduced-motion`, `a11y/focus-visible`, `a11y/inclusive-language`, `tokens/responsive-breakpoints`, `components/no-icon-fonts`. This scores three previously-unscored areas (a11y depth, responsive, assets) without writing new rules — the **first a11y sub-axes to count toward the Health Score**.
