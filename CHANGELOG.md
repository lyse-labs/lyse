...
- Dead code identified by the 2026-06-27 audit (no behavior change): `rules/pack-validator.ts` (`validatePack`), `commands/dry-run-guard.ts` (`resolveDryRun`, `dryRunFlagPresent`), `bench/taxonomy-loader.ts` (`loadTaxonomy` / `validateTaxonomy` / `Taxonomy`), the suppression `isExcluded` helper + `suppression/defaults.ts` (`DEFAULT_EXCLUDED_GLOBS`), `reliability/confidence/manifest-loader.ts` (`loadManifest`), the `codemods/safety.ts` helpers `groupByConfidence` / `groupByRule`, `ConnectorBudgetExceededError`, the retired auto-fix history helpers `appendFixEvent` / `isFirstAutoFix` (+ the `FixEvent` event type), the throwing `parseLyseConfig` variant, the dead `normalizeRemoteUrl` re-export from `identity/index.ts`, and the unread `InitOptions.firstRun` field.
