...
- **score-v2 PREVIEW channel — the moat becomes measurable (Track #71).** A read-only preview score, surfaced by `lyse explain --score` alongside the trusted v1 score, that counts a **strict superset** of the v1 stable set: the 10 **deterministic** structural sub-axes whose synthetic recall *and* precision Wilson 95 % lower bounds both clear the 0.90 promotion gate (measured on the 2026-06-17 recall-suite run) but which are **not yet promoted** into the live score — `tokens/description-coverage`, `components/no-native-shadows`, `naming/component-pascalcase`, `naming/hook-prefix`, `stories/coverage`, `ai-surface/agents-md-quality`, and the four deterministic AI-governance presence checks (`ai-marker-component-present`, `ai-loading-error-states`, `ai-content-live-region`, `feedback-control-present`). The preview lets the impact of promoting the AI-governance moat be inspected **before** any change to the trusted Health Score. New `SubAxisRecord.contributesToScoreV2` flag + `resolveScoreV2PreviewSubAxes`; the trusted (v1) score and the smoke band are **unchanged** (`contributesToScore` stays `false` for all 10). Excluded and why: `a11y/essentials` (precision LB 0.898 < 0.90), the color/spacing drift detectors (precision LB 0.893), the LLM-driven governance rules (synthetic run exercises only their deterministic fallback — real behavior needs the conformal tier), and the three recall-failing rules (`ai-tokens-reserved` 0.41, `ai-token-requires-marker` 0.10, `value-gate-doc-present` 0.64).
