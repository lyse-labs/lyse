...
- `lyse audit` now computes a deterministic score projection, attached at `meta.projection` in the JSON result: findings are grouped by fix (`fixGroup.key`, or bare `ruleId` when ungrouped), and the top 3 groups by Health Score gain are ranked with their per-group gain, distinct file count, and a `migrationScale` flag (large-blast-radius groups — sample before you sweep). Advisory only — never changes the Health Score. New config knob `advisory.migrationScaleFileCount` (default 40) tunes the `migrationScale` file-count threshold. Surfaced in the terminal in a follow-up.
