...
- **`lyse explain --score` reports the AI-Governance Maturity Level (Track #72/#155).** The output now includes a line — e.g. `AI-Governance Maturity: L3 — AI as an interaction pattern (marker component, interaction affordances)` — surfacing the deterministic maturity level (L0–L3) and which affordances were detected, alongside the Health Score. Byte-stable / deterministic (the semantic LLM tier stays opt-in, out of `explain --score`). This makes the externally-validated AI-readiness signal user-facing.
