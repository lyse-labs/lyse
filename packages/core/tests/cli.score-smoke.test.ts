...
describe("cli explain --score smoke (Track 8.10)", () => {
  it("Health Score is within [90, 96] and scoring-v1 is active", { timeout: 30_000 }, () => {
    ...
    expect(
      score,
      `Health Score ${score} is outside expected band [90, 96]. ` +
    ...
    expect(
      counted,
      `Counted findings ${counted} is outside expected band [2, 4]. ` +
        `If 0, stableSubAxes may have silently gone empty (trivial 100 regression). ` +
        `If >4, new stable sub-axes were promoted without updating this band.`,
