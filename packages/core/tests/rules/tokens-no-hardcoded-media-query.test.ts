...
describe("derived scales", () => {
  it("does not flag a breakpoint on the repo's own scale", async () => {
    const res = await runRuleWithGraph(
      "@media (min-width: 768px) { .g { color: red; } }",
      [{ id: "bp.md", axis: "breakpoints", rawValue: "768px", source: "dtcg" }],
    );
    expect(res.findings).toHaveLength(0);
  });

  it("degrades a far-off breakpoint to info/low", async () => {
    const res = await runRuleWithGraph(
  ...
  it("flags a one-step-off breakpoint as warning and names its candidate token", async () => {
    const res = await runRuleWithGraph(
      "@media (min-width: 768px) { .g { color: red; } }",
      [
        { id: "bp.sm", axis: "breakpoints", rawValue: "320px", source: "dtcg" },
        { id: "bp.lg", axis: "breakpoints", rawValue: "1280px", source: "dtcg" },
      ],
    );
    expect(res.findings).toHaveLength(1);
    expect(res.findings[0]?.severity).toBe("warning");
    expect(res.findings[0]?.confidence).toBe("medium");
    expect(res.findings[0]?.suggestion).toBe("probably `bp.sm` — verify before replacing");
  });

  it("degrades to info on a zero-token graph — no default breakpoints scale (behaviour change vs. pre-migration warning)", async () => {
    const res = await runRuleWithGraph("@media (min-width: 5000px) { .g { color: red; } }", []);
    expect(res.findings).toHaveLength(1);
    expect(res.findings[0]?.severity).toBe("info");
  });
});
