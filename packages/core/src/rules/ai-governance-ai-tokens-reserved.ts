...
export const rule: Rule = createLyseRule({
  meta: {
    ...
    fullDescription:
      "Scans token sources (`tokens.json`, `tokens/**/*.json`, `*.tokens.json`, `**/*.css` `--*` custom properties, and `**/*.scss` `$variable` declarations / `theme.$ai-*` usages) for token names that match reserved AI-marker vocabularies: Carbon (`dragon-fruit`, `*-ai-*` color tokens authored in Sass as `theme.$ai-*` and compiled to `--cds-ai-*`), AWS Cloudscape (`*-gen-ai`), Shopify Polaris (`--p-color-*-magic*`, `magic-*`), Workday Canvas (`*-ai-*` segment), and the generic leading/trailing `ai` segment. Scanning the raw SCSS `$variable` source (not just the post-transform CSS) closes the #139 gap where SCSS-authored AI tokens were invisible to a source scan. Matching is segment-anchored (split by `-`, `_`, `.`, `/`) so `rain`, `paint`, `mainColor`, `captain`, `detail` do not trigger. Emits one info finding listing up to 20 matched names when reserved tokens are present; emits nothing when none are. Severity is informational — a DS with no AI surface is not penalized here; the gating rule lives in Track 3.3 (`ai-governance/ai-token-requires-marker`).",
    helpUri:
