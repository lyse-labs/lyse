...
export interface FixOptions {
  ...
  /** Generate missing AI-readiness scaffolds (llms.txt, AGENTS.md, value-gate doc). */
...
export interface FixResult {
  ...
  /** Relative paths of AI-readiness files generated by --scaffold (empty otherwise). */
  scaffolds: string[];
  /** Relative paths of token files migrated to DTCG by --migrate-tokens (empty otherwise). */
...
export async function runFix(opt: FixOptions): Promise<FixResult> {
  ...
  // --scaffold: generate missing AI-readiness files (idempotent), behind the
  // same guards (clean tree at entry, branch created above, own commit). Dry-run
  // reports the paths without writing.
  const scaffolds: string[] = [];
  if (opt.scaffold) {
    const missing = computeMissingScaffolds(opt.cwd);
    for (const s of missing) {
      scaffolds.push(s.path);
      if (opt.dryRun) continue;
      const abs = join(opt.cwd, s.path);
      await mkdir(dirname(abs), { recursive: true });
      await writeFile(abs, s.content, "utf8");
    }
    if (!opt.dryRun && scaffolds.length > 0) {
      await commitAll(
        opt.cwd,
        `Lyse: scaffold ${scaffolds.length} AI-readiness file${scaffolds.length !== 1 ? "s" : ""} (${scaffolds.join(", ")})`,
      );
    }
  }

  // --migrate-tokens: convert legacy { value, type } token JSON to DTCG, behind
  // the same guards (clean tree at entry, branch created above, own commit).
  // Each file is self-validated against dtcg-conformance before writing; files
  // that would not produce conformant DTCG are skipped. Dry-run reports paths.
  ...
  if (opt.migrateTokens) {
    ...
    for (const m of plan.migrations) {
      migratedTokens.push(m.path);
      if (opt.dryRun) continue;
      await writeFile(join(opt.cwd, m.path), m.content, "utf8");
    }
    if (!opt.dryRun && migratedTokens.length > 0) {
      await commitAll(
        opt.cwd,
        `Lyse: migrate ${migratedTokens.length} token file${migratedTokens.length !== 1 ? "s" : ""} to DTCG (${migratedTokens.join(", ")})`,
      );
    }
  }
