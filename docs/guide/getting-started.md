...
Run `lyse handoff` — it audits, then hands the findings (grouped by drift class, with the resolved token mapping) to your coding agent to fix. Lyse never edits your code itself; the agent edits the working tree and you review the diff. The MCP server can also produce per-file patches with `suggest_fix`. The score is the start, not the goal.
