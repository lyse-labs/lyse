...
4. **`ai-governance` abstains on a repo with no AI surface** (it used to read ~100 there). `ai-governance/product-analytics` counted one opportunity per component file whatever the repo contained, while its two siblings already returned zero without an AI surface; since its findings can only fire on AI-marked files, every opportunity on a non-AI repo was clean by construction and the axis published near-perfect adoption — element-plus n=965, mantine n=3138, primer-react n=666, none of which ship an AI feature. All three rules now agree: no AI surface, no opportunities, so the axis reports `N/A` instead of a perfect mark for not having the thing being measured. The remaining denominators are still structural (project-level presence checks); a graph-derived denominator that scales with actual AI-surface size is planned for a later phase.
