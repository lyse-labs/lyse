...
- **`packages/core/src/rules/registry.ts`** — exports `ruleObjects` (all 15 rule instances) and `ruleMap` (O(1) lookup). Import from here in `fix.ts`, `share.ts`, `audit-pipeline.ts`, `codemods/safety.ts`. Do NOT build local rule arrays.
