...
- **`packages/core/src/walker.ts` + `detection/registry-evidence.ts`** — the walker excludes documentation sites (`apps/docs`, `apps/www`, `apps/*.dev`, `apps/*.com`, …), which is right for 11 of the 12 such directories across the 58 checkouts available locally and wrong for the copy-paste registry model, where the site *is* the distribution channel. magicui shipped its whole library from `apps/www/registry/` and audited **zero** files. `registryDeclaredDirs(root, patterns)` is the escape hatch and it is evidence, not another path name: a `registry.json` whose `items[].files[].path` entries resolve **relative to the directory holding it** un-excludes exactly the directories it names. That resolution test is load-bearing — magicui's tree holds four `registry.json` files and only one resolves, the built copies under `public/` and the root aggregate resolving 0 of 244 and 0 of 211. Layering is deliberate: `DEFAULT_IGNORES`, `.gitignore` and the user's `excludePaths` all outrank the evidence; only `DEFAULT_EXCLUDE_PATHS` yields to it. `REGISTRY_OVERRIDABLE_EXCLUDES` is **derived** from `DEFAULT_EXCLUDE_PATHS` (the `apps/` prefix) rather than restated, so the two cannot drift; do not widen it to the whole list — measured, that admits the identical files on the identical single repository for 180× the cost. This fixes **reach**, not **identity**: `identifyDsFamily` still rejects magicui's only package as `app-or-site-directory`, so its component inventory is empty.
...
- **`packages/core/src/cli/unknown-flags.ts` + `enum-flags.ts`** — the CLI fails closed on a flag it does not know (`assertKnownFlags`, unknown *name*) and on a value a flag does not accept (`assertEnumValues`, unknown *value*), both via `ScopeError` → exit 64. **Adding a `--format` to the dispatch chain in `cli.ts` means adding it to `AUDIT_ENUM_FLAGS` too** — the chain is if/else-if and cannot be enumerated, so the accepted set lives in `enum-flags.ts` and nowhere else. Failing to update it rejects the new format loudly on first use, which is the intended direction: `--scope New` used to widen the audit to the whole tree, skip `evaluateGate` entirely and exit 0 (#276).
